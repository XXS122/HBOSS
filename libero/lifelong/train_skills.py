...
from libero.lifelong.handoff_metrics import select_task_ids
...
def main(hydra_cfg):
    ...
    selected_ids = select_task_ids(n_manip_tasks, cfg.get("train_task_ids"))
    manip_datasets = {}
    ...
    for i in selected_ids:
        ...
        manip_datasets[i] = task_i_dataset
    ...
    # prepare demos
    datasets = {i: SequenceVLDataset(ds, task_embs[i]) for i, ds in manip_datasets.items()}

    n_demos = [data.n_demos for data in datasets.values()]
    n_sequences = [data.total_num_sequences for data in datasets.values()]
    ...
    for i in selected_ids:
        ...
        if cfg.eval.eval:
            ...
            L = evaluate_loss(cfg, policy_starter, benchmark, [datasets[i]])
            t2 = time.time()
            ...
            t3 = time.time()
            result_summary["L_conf_mat"][i][i] = L[0]
            result_summary["S_conf_mat"][i][i] = S[0]
            if cfg.use_wandb:
