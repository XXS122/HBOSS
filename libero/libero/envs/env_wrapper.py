...
class SequentialEnv(OffScreenRenderEnv):
    ...
    def reset(self):
        ...
        self.complete_task = []
        self.complete_id = -1
        self.task_dones = [False for _ in range(self.n_tasks)]
        self.obs = self.env_ls[0].reset()
        self.reward, self.done, self.info = 0.0, False, {}
        return self.obs
    ...
    def step(self, action):
        ...
        if done:
            ...
            if self.task_id != (self.n_tasks - 1):
                ...
                obs = self.env_ls[self.task_id].set_init_state(crr_env_state)
